...
class Uploader(BaseUploader):
    ...
    def create_url(self):
        ...
        city = random.choice(ipvanish['city'])
        proxies = {
            "http": f"socks5://{ipvanish['accounts']}:{ipvanish['password']}@{city}:{ipvanish['port']}",
            "https": f"socks5://{ipvanish['accounts']}:{ipvanish['password']}@{city}:{ipvanish['port']}",
        }
        resp = requests.post(
            self.client.url, headers=self.get_url_creation_headers(), proxies=proxies)
